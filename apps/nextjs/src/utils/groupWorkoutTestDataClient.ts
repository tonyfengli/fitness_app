...
class GroupWorkoutTestDataClientImpl implements GroupWorkoutTestDataClient {
  ...
  async listSessions(): Promise<GroupSessionSummary[]> {
    ...
    sessions.forEach((s: GroupSessionSummary, i: number) => {
      ...
      const satisfactionScore = satisfaction.fullyMet / 
        (satisfaction.fullyMet + satisfaction.partiallyMet + satisfaction.notMet);
  ...
  async analyzeCohesion(sessionId: string): Promise<CohesionAnalysis> {
    ...
    const avgSatisfaction = analysis.clientSatisfaction.reduce(
      (sum, c) => sum + (c.actualRatio / c.targetRatio), 0
    ...
    const lowQualityBlocks = analysis.blockAnalysis.filter(b => b.sharedExerciseQuality < 1.5);
