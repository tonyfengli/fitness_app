...
export const debugAuth = async () => {
  ...
  const sessionLogs = logs.filter(
    (log) => log.event === "Session raw response",
  );
  ...
  try {
    ...
    const clientRes = await fetch("/api/auth/get-session", {
      credentials: "include",
    });
...
function generateRecommendation(logs: any[]) {
  const lastSession = logs
    .filter((log) => log.event === "Session raw response")
    .pop();
...
export const enableAutoCapture = () => {
  ...
  setInterval(() => {
    if (window.location.href !== lastUrl) {
      ...
      FrontendDebugClient.logNavigation(
        lastUrl,
      ...
      setTimeout(() => {
        const logs = FrontendDebugClient.getLogs();
        const recentAuth = logs.filter(
          (log) => log.timestamp > new Date(Date.now() - 1000).toISOString(),
        );
...
export const debugClients = async () => {
  ...
  try {
    ...
    console.log(
      'To retrieve: JSON.parse(localStorage.getItem("debug-clients-data"))',
    );
