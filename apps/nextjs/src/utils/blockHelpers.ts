...
export function extractBlockInfo(exercises: any[]): BlockInfo[] {
  ...
  const hasLegacyFlags = exercises.some(
    (ex) =>
      "isSelectedBlockA" in ex ||
      "isSelectedBlockB" in ex ||
      "isSelectedBlockC" in ex ||
      "isSelectedBlockD" in ex,
  );
  ...
  if (hasLegacyFlags) {
    ...
    return [
      ...
      },
      {
        ...
        exercises: exercises.filter((ex) =>
          ex.functionTags?.includes("accessory"),
        ),
        color: "purple",
        maxCount: 8,
      },
