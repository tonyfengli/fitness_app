...
export function GroupTestDataInitializer() {
  useEffect(() => {
    ...
    const groupTestData = {
      async enable() {
        const response = await fetch('/api/debug/group-workout-data/enable', { method: 'POST' });
        if (!response.ok) throw new Error('Failed to enable group workout test data');
        console.log('✅ Group workout test data logging enabled');
      },
      
      async disable() {
        const response = await fetch('/api/debug/group-workout-data/disable', { method: 'POST' });
        if (!response.ok) throw new Error('Failed to disable group workout test data');
      ...
      async listSessions() {
        ...
        sessions.forEach((s: any, i: number) => {
          ...
          const satisfactionScore = satisfaction.fullyMet / 
            (satisfaction.fullyMet + satisfaction.partiallyMet + satisfaction.notMet) || 0;
          
          console.log(`${i + 1}. ${s.sessionId.substring(0, 8)}...`);
          console.log(`   Clients: ${s.clientCount} | Template: ${s.templateType}`);
          console.log(`   Cohesion: ${(satisfactionScore * 100).toFixed(0)}% satisfied`);
      ...
      async getSession(sessionId: string) {
        const response = await fetch(`/api/debug/group-workout-data/session/${sessionId}`);
      ...
      async getLatest() {
        ...
        console.log(`- Phase 1&2: ${data.summary.phaseBreakdown.phase1_2_parallel}ms`);
      ...
      async analyzeCohesion(sessionId: string) {
        ...
        for (const block of session.phaseB.slotAllocationDetails) {
          ...
          const excellentCount = block.candidateStats.sharedCandidatesQuality.excellent || 0;
          const goodCount = block.candidateStats.sharedCandidatesQuality.good || 0;
          const quality = (excellentCount * 2 + goodCount) / Math.max(1, actualShared);
          
          // Get top shared exercises
          const blockData = session.phase2_5.blockScoringData.find((b: any) => b.blockId === block.blockId);
          const topShared = blockData?.cohesionBonuses
            .slice(0, 3)
            .map((e: any) => e.exerciseName) || [];
          
          analysis.blockAnalysis.push({
            blockId: block.blockId,
            targetSharedRatio: block.allocation.targetSharedSlots / block.allocation.totalSlots,
            actualSharedRatio: block.allocation.finalSharedSlots / block.allocation.totalSlots,
        ...
        for (const status of session.phaseB.cohesionAnalysis.finalStatus) {
          const client = session.groupContext.clients.find((c: any) => c.user_id === status.clientId);
          analysis.clientSatisfaction.push({
            ...
            satisfactionLevel: status.satisfied ? 'Satisfied' : 
              status.actualSharedRatio / status.targetSharedRatio > 0.7 ? 'Partially Met' : 'Not Met'
          });
        }
        
        // Calculate overall score
        const avgSatisfaction = analysis.clientSatisfaction.reduce(
          (sum: number, c: any) => sum + (c.actualRatio / c.targetRatio), 0
        ...
        if (analysis.overallCohesionScore < 0.8) {
          analysis.recommendations.push('Consider adjusting cohesion ratios for better satisfaction');
        }
        
        const lowQualityBlocks = analysis.blockAnalysis.filter((b: any) => b.sharedExerciseQuality < 1.5);
        ...
        console.log(`Overall Score: ${(analysis.overallCohesionScore * 100).toFixed(1)}%`);
        console.log('\n📊 Block Analysis:');
        analysis.blockAnalysis.forEach((b: any) => {
          console.log(`Block ${b.blockId}: ${(b.actualSharedRatio * 100).toFixed(0)}% shared (target: ${(b.targetSharedRatio * 100).toFixed(0)}%)`);
        ...
        analysis.clientSatisfaction.forEach((c: any) => {
          console.log(`${c.clientName}: ${c.satisfactionLevel} (${(c.actualRatio * 100).toFixed(0)}% shared)`);
        });
      ...
      async compareClients(sessionId: string) {
        ...
        // Analyze each client
        for (const client of session.phaseA.clients) {
          const clientExercises = new Set(client.phase2.topExercises.map((e: any) => e.id));
          ...
          const diversity = unique.length / Math.max(1, client.phase2.topExercises.length);
        ...
        comparisons.forEach((c: any) => {
          console.log(`\n${c.clientName}:`);
          console.log(`  Preferences: ${c.preferences.intensity} intensity, ${c.preferences.sessionGoal} goal`);
          console.log(`  Targets: ${c.preferences.muscleTargets.join(', ') || 'none'}`);
          console.log(`  Avoid Joints: ${c.preferences.avoidJoints.join(', ') || 'none'}`);
          console.log(`  Diversity Score: ${(c.workoutDiversity * 100).toFixed(0)}%`);
          console.log(`  Shared with all: ${c.exerciseOverlap.sharedWithAll.length} exercises`);
      ...
      async clearSessions() {
        const response = await fetch('/api/debug/group-workout-data/clear', { method: 'DELETE' });
    ...
    console.log('🏋️ Group workout test data utilities loaded. Use groupTestData.* to access.');
  }, []);
