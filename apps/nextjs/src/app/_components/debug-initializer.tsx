...
export function DebugInitializer() {
  useEffect(() => {
    if (process.env.NODE_ENV !== "production") {
      ...
      (window as any).FrontendDebugClient = import(
        "~/utils/frontendDebugClient"
      ).then((m) => m.FrontendDebugClient);

      // Only show console messages if explicitly enabled
      if (isDebugEnabled()) {
        console.log(
          "🛠️ Debug tools initialized (disabled by default for performance)",
        );
        console.log("Commands:");
        console.log(
          "  • window.frontendDebug.setEnabled(true) - Enable debug logging",
        );
