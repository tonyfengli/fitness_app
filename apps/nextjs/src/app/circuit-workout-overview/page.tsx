...
function CircuitWorkoutOverviewContent() {
  ...
  // Music section expansion state
  const [expandedMusicRounds, setExpandedMusicRounds] = useState<Set<string>>(new Set());
          ...
          <>
            ...
            {roundsData.map((round, roundIndex) => {
              ...
              >
                ...
                {/* Music Section - Expandable */}
                {roundMusic && (
                  <div className="mt-4">
                    <button
                      onClick={() => {
                        const newExpanded = new Set(expandedMusicRounds);
                        if (newExpanded.has(round.roundName)) {
                          newExpanded.delete(round.roundName);
                        } else {
                          newExpanded.add(round.roundName);
                        }
                        setExpandedMusicRounds(newExpanded);
                      }}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm font-medium text-gray-600 dark:text-gray-400">Music</span>
                        </div>
                        <svg
                          className={`w-4 h-4 text-gray-400 dark:text-gray-500 transition-transform ${
                            expandedMusicRounds.has(round.roundName) ? 'rotate-180' : ''
                          }`}
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                    ...
                    {/* Expanded content */}
                    {expandedMusicRounds.has(round.roundName) && (
