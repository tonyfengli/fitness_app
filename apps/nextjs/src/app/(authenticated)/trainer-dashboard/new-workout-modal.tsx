...
export default function NewWorkoutModal({
  ...
  const [filteredExercises, setFilteredExercises] =
    useState<FilteredExercisesResult | null>(null);
  ...
  const handleGenerateWorkout = useCallback(async () => {
    if (!filteredExercises) {
      setError(
        "No filtered exercises available. Please go back and try again.",
      );
      return;
    }
    ...
    try {
      ...
      if (filteredExercises.blocks) {
        ...
        if (filteredExercises.blocks.blockA) {
          exercisesForLLM.blockA = filteredExercises.blocks.blockA.map(
            (ex: any) => ({
              id: ex.id,
              name: ex.name,
              score: ex.score || 0,
        ...
        if (filteredExercises.blocks.blockB) {
          exercisesForLLM.blockB = filteredExercises.blocks.blockB.map(
            (ex: any) => ({
              id: ex.id,
              name: ex.name,
              score: ex.score || 0,
        ...
        if (filteredExercises.blocks.blockC) {
          exercisesForLLM.blockC = filteredExercises.blocks.blockC.map(
            (ex: any) => ({
              id: ex.id,
              name: ex.name,
              score: ex.score || 0,
        ...
        if (filteredExercises.blocks.blockD) {
          exercisesForLLM.blockD = filteredExercises.blocks.blockD.map(
            (ex: any) => ({
              id: ex.id,
              name: ex.name,
              score: ex.score || 0,
      ...
      await generateWorkoutMutation.mutateAsync({
        userId: clientId,
        templateType: workoutParams.template as
          | "standard"
          | "circuit"
          | "full_body",
    ...
    if (currentStep === 1) {
      ...
      try {
        const result = await filterMutation.mutateAsync({
          ...
          template: workoutParams.template as
            | "standard"
            | "circuit"
            | "full_body",
  ...
  return (
    <>
      ...
      >
        {/* Modal */}
        <div
          className={`flex h-full items-center justify-center ${isFullscreen ? "" : "p-4"}`}
        >
          <div
            className={`flex flex-col overflow-hidden bg-white shadow-xl transition-all duration-300 ${
              isFullscreen
                ? "h-full w-full rounded-none"
                : "h-[85vh] w-full max-w-2xl rounded-2xl"
            }`}
