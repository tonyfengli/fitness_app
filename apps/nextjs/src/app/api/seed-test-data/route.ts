...
import { user, Business, Workout, WorkoutExercise, exercises } from "@acme/db/schema";
...
export async function POST(request: Request) {
  try {
    ...
    if (!businessId) {
      return NextResponse.json({ error: "Trainer has no associated business" }, { status: 404 });
    }
    ...
    if (clientEmail) {
      ...
      if (!testClient) {
        return NextResponse.json({ error: `Client ${clientEmail} not found in business` }, { status: 404 });
      }
    } else {
      ...
      if (!testClient) {
        return NextResponse.json({ error: "No clients found in business. Please create a client first." }, { status: 404 });
      }
    }
    ...
    if (availableExercises.length < 9) {
      return NextResponse.json({ error: "Not enough exercises in database" }, { status: 400 });
    }
    ...
    if (existingWorkouts.length > 0) {
      await db.delete(Workout).where(
        eq(Workout.id, existingWorkouts[0].id)
      );
    }
    ...
    const exercisesToAdd = [
      // Block A - 3 exercises
      { exercise: availableExercises[0], groupName: 'Block A', order: 1, sets: 3 },
      { exercise: availableExercises[1], groupName: 'Block A', order: 2, sets: 3 },
      { exercise: availableExercises[2], groupName: 'Block A', order: 3, sets: 3 },
      // Block B - 3 exercises
      { exercise: availableExercises[3], groupName: 'Block B', order: 1, sets: 4 },
      { exercise: availableExercises[4], groupName: 'Block B', order: 2, sets: 4 },
      { exercise: availableExercises[5], groupName: 'Block B', order: 3, sets: 4 },
      // Block C - 3 exercises
      { exercise: availableExercises[6], groupName: 'Block C', order: 1, sets: 3 },
      { exercise: availableExercises[7], groupName: 'Block C', order: 2, sets: 3 },
      { exercise: availableExercises[8], groupName: 'Block C', order: 3, sets: 3 },
    ];
