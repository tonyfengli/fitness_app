...
async function globalSetup(config: FullConfig) {
  ...
  const testEmail = process.env.TEST_TRAINER_EMAIL || 'test-trainer@example.com';
  ...
  try {
    ...
    const response = await page.request.post('http://localhost:3000/api/seed-test-data', {
      data: { 
    ...
    if (response.ok()) {
      ...
      console.log(`✅ Test data seeded: ${data.workout.notes} with ${data.workout.exerciseCount} exercises`);
    } else {
