...
export async function selectClient(page: Page, clientName: string) {
  ...
  await page.waitForSelector('nav[aria-label="Client navigation"]', { state: 'visible' });
  
  // Wait for clients to load (check that "Loading clients..." is gone)
  await page.waitForFunction(() => {
    const mainContent = document.querySelector('main');
    return mainContent && !mainContent.textContent?.includes('Loading clients...');
  }, { timeout: 10000 });
  ...
  if (await clientButton.isVisible()) {
    ...
    await page.waitForSelector(`h1:has-text("${clientName}")`, { state: 'visible' });
  } else {
