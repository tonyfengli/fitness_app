...
test.describe("Workout Management", () => {
  ...
  test("should delete an exercise from workout block", async ({ page }) => {
    ...
    const exerciseName = await firstExercise
      .locator('[data-testid="exercise-name"]')
      .textContent();

    // Count exercises in the block before deletion
    const exerciseBlock = firstExercise.locator(
      'xpath=ancestor::*[@data-testid="section-block"]',
    );
    const initialExerciseCount = await exerciseBlock
    ...
    await page.waitForSelector('[data-testid="exercise-menu"]', {
      state: "visible",
    });
  ...
  test("should delete an entire workout", async ({ page }) => {
    ...
    const workoutMenuButton = page
      .locator('[data-testid="workout-menu-button"]')
      .first();
    await expect(workoutMenuButton).toBeVisible();

    // Count initial workouts
    const initialWorkoutCount = await page
    ...
    // Wait for menu to appear
    await page.waitForSelector('[data-testid="workout-menu"]', {
      state: "visible",
    });
  ...
  test("should delete a section block", async ({ page }) => {
    ...
    const sectionName = await sectionBlock
      .locator('[data-testid="section-name"]')
      .textContent();
    ...
    // Wait for menu to appear
    await page.waitForSelector('[data-testid="section-menu"]', {
      state: "visible",
    });
  ...
  test("should duplicate workout for today", async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="duplicate-modal"]', {
      state: "visible",
    });
    ...
    await expect(page.locator('[data-testid="duplicate-modal"]')).toContainText(
      "Workout Summary",
    );
  ...
  test("should edit exercise manually", async ({ page }) => {
    ...
    const originalExerciseName = await firstExercise
      .locator('[data-testid="exercise-name"]')
      .textContent();
    ...
    await page.waitForSelector('[data-testid="edit-exercise-modal"]', {
      state: "visible",
    });
    ...
    await page.waitForSelector('[data-testid="exercise-search-input"]', {
      state: "visible",
    });
    ...
    await page.waitForSelector('[data-testid="sets-input"]', {
      state: "visible",
    });
  ...
  test("should change exercise sequence with move up/down", async ({
    page,
  }) => {
    ...
    if (exerciseCount < 3) {
      throw new Error(
        `Expected at least 3 exercises in Block A for move up/down test, but found ${exerciseCount}`,
      );
    }
    ...
    const exerciseName = await secondExercise
      .locator('[data-testid="exercise-name"]')
      .textContent();
    const initialSequence = await secondExercise
      .locator('[data-testid="exercise-sequence"]')
      .textContent();
    ...
    await secondExercise
      .locator('[data-testid="exercise-menu-button"]')
      .click();
    ...
    const newSequenceAfterDown = await exercises
      .nth(2)
      .locator('[data-testid="exercise-sequence"]')
      .textContent();
    ...
    const movedExerciseName = await exercises
      .nth(2)
      .locator('[data-testid="exercise-name"]')
      .textContent();
    ...
    const finalSequence = await exercises
      .nth(1)
      .locator('[data-testid="exercise-sequence"]')
      .textContent();
    ...
    const finalExerciseName = await exercises
      .nth(1)
      .locator('[data-testid="exercise-name"]')
      .textContent();
  ...
  test("should show correct move options based on position", async ({
    page,
  }) => {
    // Get the first workout card (the test workout)
    const firstWorkout = page.locator('[data-testid="workout-card"]').first();
    ...
    await expect(
      page.locator('[data-testid="move-up-option"]'),
    ).not.toBeVisible();
    // Should have Move Down option
    await expect(
      page.locator('[data-testid="move-down-option"]'),
    ).toBeVisible();
  ...
  test("should add exercise to block", async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="add-exercise-modal"]', {
      state: "visible",
    });
    await page.waitForTimeout(1000); // Let modal fully render
    ...
    const finalExerciseCount = await blockA
      .locator('[data-testid="exercise-row"]')
      .count();
