...
export function getOldMusclesForConsolidated(consolidatedMuscle: ConsolidatedMuscle): string[] {
  ...
  return REVERSE_MUSCLE_MAPPING[lowerMuscle] || REVERSE_MUSCLE_MAPPING[consolidatedMuscle] || [consolidatedMuscle];
}
...
export function exerciseMatchesMusclePreference(
  ...
  console.log(`[muscleMapping] Checking: "${exerciseMuscle}" vs preference "${userPreference}"`);
  console.log(`[muscleMapping] Old muscles for "${userPreference}": [${oldMuscles.join(', ')}]`);
