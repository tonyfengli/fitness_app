...
export function filterByStrengthEnhanced(
  ...
  const filtered = exercises.filter(exercise => {
    ...
    if (!isAllowed) {
      exclusionTracker.addExclusion(exercise, `strength_too_high:${exercise.strengthLevel}_exceeds_${strengthLevel}`);
    }
    return isAllowed;
  });
  
  const duration = performance.now() - startTime;
  debugLogger.log('filtering', `Strength filtering complete`, {
...
export function filterBySkillEnhanced(
  ...
  const filtered = exercises.filter(exercise => {
    ...
    if (!isAllowed) {
      exclusionTracker.addExclusion(exercise, `complexity_too_high:${exercise.complexityLevel}_exceeds_${skillLevel}`);
    }
    return isAllowed;
  });
  
  const duration = performance.now() - startTime;
  debugLogger.log('filtering', `Skill filtering complete`, {
    excluded: exercises.length - filtered.length,
    remaining: filtered.length
  }, exercises.length - filtered.length);
  debugLogger.logPerformance(debugLogger.getLogs().length, duration);
...
export function filterByAvoidJointsEnhanced(
  ...
  const filtered = exercises.filter(exercise => {
    const hasRestrictedJoint = exercise.loadedJoints?.some(joint => 
      avoidJoints.includes(joint)
    ) ?? false;
    
    if (hasRestrictedJoint) {
      const restrictedJoints = exercise.loadedJoints?.filter(joint => 
        avoidJoints.includes(joint)
      ) ?? [];
      exclusionTracker.addExclusion(exercise, `joint_restriction:${restrictedJoints.join(',')}`);
    }
  ...
  const duration = performance.now() - startTime;
  debugLogger.log('filtering', `Joint restriction filtering complete`, {
    excluded: exercises.length - filtered.length,
    remaining: filtered.length
  }, exercises.length - filtered.length);
  debugLogger.logPerformance(debugLogger.getLogs().length, duration);
...
export function filterByExcludeEnhanced(
  ...
  const duration = performance.now() - startTime;
  debugLogger.log('filtering', `Exclude filtering complete`, {
    excluded: exercises.length - filtered.length,
    remaining: filtered.length
  }, exercises.length - filtered.length);
  debugLogger.logPerformance(debugLogger.getLogs().length, duration);
...
export function applyAllFiltersEnhanced(
  ...
  if (filters.avoidJoints && filters.avoidJoints.length > 0) {
    includedExercises = filterByAvoidJointsEnhanced(includedExercises, filters.avoidJoints);
    standardFiltered = filterByAvoidJointsEnhanced(standardFiltered, filters.avoidJoints);
  }
