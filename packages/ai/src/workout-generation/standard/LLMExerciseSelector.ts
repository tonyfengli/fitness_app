...
export interface LLMSelectionConfig {
  ...
  groupContext: Array<{
    // Simplified group info for prompts
    clientId: string;
    name: string;
...
export class LLMExerciseSelector {
  ...
  async selectExercisesForAllClients(
    ...
    console.log(
      "[LLM Timing] Starting parallel requests for",
      clientInputs.length,
      "clients at",
    ...
    const selectionPromises = clientInputs.map((input, index) => {
      console.log(
        `[LLM Timing] Creating promise for client ${index + 1}/${clientInputs.length}: ${input.clientId}`,
      );
      return this.selectExercisesForClient(input).catch((error) => {
        console.error(
          `[LLMExerciseSelector] LLM selection failed for client ${input.clientId}:`,
          error,
        );
  ...
  private async selectExercisesForClient(
    ...
    console.log(
      `[LLM Timing] Starting LLM request for client ${input.clientId} at`,
      new Date().toISOString(),
    );
    ...
    const promptBuilder = new ClientExerciseSelectionPromptBuilder({
      client: input.client,
      workoutType:
        (input.client.workoutType as WorkoutType) ||
    ...
    try {
      ...
      console.log(
        `[Timestamp] Phase 1 LLM call started for client ${input.clientId} at: ${llmStartISO}`,
      );

      const response = await this.llm.invoke([
        new SystemMessage(prompt),
        new HumanMessage(
          "Please select the exercises according to the instructions above.",
        ),
      ]);
      ...
      console.log(
        `[Timestamp] Phase 1 LLM call completed for client ${input.clientId} at: ${llmEndISO}`,
      );
      ...
      console.log(
        `[LLM Timing] Completed LLM request for client ${input.clientId} at`,
        new Date().toISOString(),
      );
      ...
      if (jsonMatch) {
        ...
        try {
          ...
          console.error(
            `[LLMExerciseSelector] Failed to parse LLM response as JSON:`,
            parseError,
          );
  ...
  private validateAndTransformResponse(
    ...
    const selectedExercises = llmResponse.selectedExercises.map(
      (selection: any) => {
        ...
        if (!exercise) {
          throw new Error(
            `Exercise "${selection.exerciseName || selection.exerciseId}" not found in candidates`,
          );
        }
    ...
    const summary = {
      ...
      sharedExercises:
        llmResponse.summary?.sharedExercises ||
      ...
      overallReasoning:
        llmResponse.summary?.overallReasoning || "No summary provided",
    };

    // Validate count
    const expectedCount = this.getExpectedExerciseCount(
      input.client.intensity || "moderate",
      input.preAssigned.length,
    );
  ...
  private createFallbackSelection(
    ...
    const movementPatterns = [
      ...new Set(
        selectedExercises
          .map((s) => s.exercise.movementPattern)
          .filter(Boolean),
      ),
    ] as string[];
   ...
   */
  private getExpectedExerciseCount(
