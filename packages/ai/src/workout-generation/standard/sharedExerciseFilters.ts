...
export function categorizeSharedExercises(
  ...
  for (const exercise of sharedExercises) {
    ...
    const hasCoreOrCapacity = exercise.functionTags?.some(
 ...
 */
export function isCoreOrFinisherExercise(exercise: GroupScoredExercise): boolean {
  return exercise.functionTags?.some(
    tag => tag === 'core' || tag === 'capacity'
  ) ?? false;
}
