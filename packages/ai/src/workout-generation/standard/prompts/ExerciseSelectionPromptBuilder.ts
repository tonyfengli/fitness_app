...
\`\`\``;
  ...
  private getClientName(clientId: string): string {
    const client = this.groupContext.clients.find(
      (c) => c.user_id === clientId,
    );
    return client?.name || clientId;
  }

  private groupSharedExercisesByClientCount(): Map<
    number,
    GroupScoredExercise[]
  > {
  ...
  private formatScoreFactors(breakdown: any): string {
    ...
    if (breakdown.includeExerciseBoost > 0) {
      factors.push(
        `client requested (+${breakdown.includeExerciseBoost.toFixed(1)})`,
      );
    }
    ...
    if (breakdown.muscleTargetBonus > 0) {
      ...
      factors.push(
        `targets muscle${isPrimary ? "" : " (secondary)"} (+${breakdown.muscleTargetBonus.toFixed(1)})`,
      );
    }

    if (breakdown.muscleLessenPenalty < 0) {
      factors.push(
        `lessens muscle (${breakdown.muscleLessenPenalty.toFixed(1)})`,
      );
    }
    ...
    if (breakdown.intensityAdjustment !== 0) {
      ...
      factors.push(
        `intensity match (${sign}${breakdown.intensityAdjustment.toFixed(1)})`,
      );
    }
