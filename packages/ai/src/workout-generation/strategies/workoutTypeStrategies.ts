 ...
 */
export function getWorkoutTypeStrategy(
...
export function processPreAssignments(
  ...
  const sortedRules = [...strategy.preAssignmentRules].sort(
  ...
  for (const rule of sortedRules) {
    ...
    switch (rule.type) {
      ...
      case "favorite":
        ...
        if (
          ...
          if (
            selectedUpperWithTies.length === 0 &&
            selectedLowerWithTies.length === 2
          ) {
            // No upper body favorites available, using 2 lower body
          } else if (
            selectedLowerWithTies.length === 0 &&
            selectedUpperWithTies.length === 2
          ) {
          ...
          [...selectedUpperWithTies, ...selectedLowerWithTies].forEach(
            (item) => {
      ...
      case "muscle_target":
        if (rule.filter) {
          candidates = exercises.filter(
            (ex) => !usedIds.has(ex.id) && rule.filter!(ex, client),
          );
        }
        break;
    }
    ...
    const toAdd = candidates.slice(
      0,
      Math.min(rule.count, strategy.maxPreAssignments - preAssigned.length),
    );
  ...
  console.log(
    `  Pre-assigned ${preAssigned.length} exercises for workout type ${workoutType}`,
  );
