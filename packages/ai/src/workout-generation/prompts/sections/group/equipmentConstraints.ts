...
export function generateEquipmentConstraints(equipment: Equipment): string {
  ...
  if (equipment.cable_machine > 0)
    limited.push(`${equipment.cable_machine} cable machine`);
  if (equipment.landmine > 0) limited.push(`${equipment.landmine} landmine`);
  if (equipment.row_machine > 0)
    limited.push(`${equipment.row_machine} row machine`);
  ...
  if (equipment.kettlebells > 0)
    available.push(`${equipment.kettlebells} kettlebells`);
  if (equipment.medicine_balls > 0) available.push("medicine balls");
  if (equipment.dumbbells === "unlimited")
    available.push("dumbbells (unlimited)");
