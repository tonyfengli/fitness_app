...
export function generateBMFGroupPrompt(config: GroupWorkoutConfig): string {
  ...
  // Header
  sections.push(
    `You are a group fitness coordinator planning exercises for rounds 3 and 4 of a workout for ${clients.length} clients.`,
  );
  sections.push("");
  ...
  sections.push(
    "- Round 3: Client goal-focused strength work (ideally including push for balance)",
  );
  sections.push(
  ...
  clients.forEach((client) => {
    ...
    if (deterministicAssignments?.Round3) {
      used += deterministicAssignments.Round3.filter(
        (a) => a.clientId === client.user_id,
      ).length;
    }
    if (deterministicAssignments?.FinalRound) {
      used += deterministicAssignments.FinalRound.filter(
        (a) => a.clientId === client.user_id,
      ).length;
    }
  ...
  if (
    deterministicAssignments?.Round3 ||
    deterministicAssignments?.FinalRound
  ) {
    sections.push("## Pre-Assigned Exercises:");
    if (
      deterministicAssignments?.Round3 &&
      deterministicAssignments.Round3.length > 0
    ) {
      sections.push("Round 3:");
      deterministicAssignments.Round3.forEach((assignment) => {
        const reason =
          assignment.reason === "client_request"
            ? "CLIENT REQUEST"
            : "MUSCLE TARGET";
        sections.push(
          `- ${assignment.clientName}: ${assignment.exercise} (${reason} - ALREADY ASSIGNED)`,
        );
      });
    }
    if (
      deterministicAssignments?.FinalRound &&
      deterministicAssignments.FinalRound.length > 0
    ) {
  ...
  clients.forEach((client) => {
    ...
    sections.push(
      `- ${client.name}: Targets ${targets} ${covered.length > 0 ? "(covered in: " + covered.join(", ") + ")" : "❌ MUST ASSIGN"}`,
    );
  });
  sections.push("");
  ...
  clients.forEach((client) => {
    ...
    const r3Assigned =
      deterministicAssignments?.Round3?.filter(
        (a) => a.clientId === client.user_id,
      ).length || 0;
    const r4Assigned =
      deterministicAssignments?.FinalRound?.filter(
        (a) => a.clientId === client.user_id,
      ).length || 0;

    sections.push(
      `- ${client.name}: ${remaining} left (${1 - r3Assigned} in R3, ${1 - r4Assigned} in R4)`,
    );
  });
  sections.push("");
  ...
  sections.push(
    "**Primary Goal: Prioritize exercises that align with each client's muscle targets and fitness goals**",
  );
  sections.push(
    "**Secondary Goal: Maintain movement pattern balance (include pushing movements to balance the pulling from Round 2)**",
  );
  ...
  sections.push(
    "- Client lessening shoulders → avoid overhead work, choose bench/horizontal movements",
  );
  sections.push("");
  ...
  sections.push(
    "**Primary Goal: Select exercises that target client goals while incorporating core/metabolic work**",
  );
  sections.push(
    "**Secondary Goal: End the workout strong with burnout-style finishers (high reps, metabolic conditioning)**",
  );
  sections.push("Examples:");
  sections.push(
    "- Core-focused finishers: plank variations, dead bugs, carries",
  );
  sections.push(
  ...
  sections.push(
    "- Client needing conditioning → prioritize metabolic/cardio finishers",
  );
  sections.push("");
  sections.push("### Shared Options:");
  ...
  sections.push("## Task:");
  sections.push(
    "Assign exercises for R3 and R4 following these MANDATORY requirements:",
  );
  sections.push("");
  ...
  sections.push(
    "2. Each client MUST have at least 1 shared exercise (across R1-R4)",
  );
  sections.push(
    '3. Respect all "lessen" requests (never assign exercises for those muscles)',
  );
  sections.push(
  ...
  sections.push(
    "2. R3: Focus on strength work, ideally including push movements",
  );
  sections.push("3. R4: Create a strong finish with core/metabolic work");
  sections.push(
    "4. Use shared exercises when multiple clients have similar needs",
  );
  sections.push("");
