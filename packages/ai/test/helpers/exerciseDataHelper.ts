...
export class ExerciseDataHelper {
  ...
  constructor() {
    ...
    if (!dataPath) {
      throw new Error(
        `Exercise data not found. Tried paths: ${possiblePaths.join(", ")}. Run 'npm run export-exercises' first.`,
      );
    }
  ...
  getBasicTestSet(count = 30): Exercise[] {
    ...
    for (const tag of functionTags) {
      const exercise = this.allExercises.find(
        (ex) => ex.functionTags?.includes(tag) && !used.has(ex.id),
      );
      if (exercise) {
    ...
    for (const level of strengthLevels) {
      const exercise = this.allExercises.find(
        (ex) => ex.strengthLevel === level && !used.has(ex.id),
      );
      if (exercise) {
        selected.push(exercise);
        used.add(exercise.id);
    ...
    for (const pattern of patterns) {
      const exercise = this.allExercises.find(
        (ex) => ex.movementPattern === pattern && !used.has(ex.id),
      );
      if (exercise) {
        selected.push(exercise);
        used.add(exercise.id);
  ...
  getEdgeCaseSet(): {
    ...
    return {
      ...
      withJointLoads: this.allExercises.filter(
        (ex) => ex.loadedJoints && ex.loadedJoints.length > 0,
      ),
      noSecondaryMuscles: this.allExercises.filter(
        (ex) => !ex.secondaryMuscles || ex.secondaryMuscles.length === 0,
      ),
      multiFunction: this.allExercises.filter(
        (ex) => ex.functionTags && ex.functionTags.length > 1,
      ),
    };
  }
  ...
  getConstraintTestSet(): Exercise[] {
    ...
    const sorted = [...this.allExercises].sort((a, b) => {
      const aUnique =
        !usedPatterns.has(a.movementPattern) &&
        !usedMuscles.has(a.primaryMuscle);
      const bUnique =
        !usedPatterns.has(b.movementPattern) &&
        !usedMuscles.has(b.primaryMuscle);
    ...
    for (const exercise of sorted) {
      ...
      if (
        usedPatterns.has(exercise.movementPattern) &&
