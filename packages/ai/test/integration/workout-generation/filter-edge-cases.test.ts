...
describe("Filter Edge Cases & Validation", () => {
  ...
  describe("Skill Level Cascading", () => {
    it("should apply cascading inclusion for skill levels (higher includes lower)", async () => {
      ...
      const skillLevels = result.filteredExercises.map(
        (ex) => ex.complexityLevel,
      );
    ...
    it("should handle skill cascading independently from strength cascading", async () => {
      ...
      const hasHighStrength = result.filteredExercises.some(
        (ex) => ex.strengthLevel === "high",
      );
      const hasHighSkill = result.filteredExercises.some(
        (ex) => ex.complexityLevel === "high",
      );
    ...
    it("should handle exercises with null/undefined strength level", async () => {
      const invalidExercises: Exercise[] = [
        ...
        createTestExerciseWithOverrides({
          ...testExercises[1],
          id: "undef1",
          strengthLevel: undefined as any,
        }),
        createTestExerciseWithOverrides({
          ...testExercises[2],
          id: "valid1",
      ...
      expect(result.filteredExercises.every((ex) => ex.strengthLevel)).toBe(
        true,
      );
    });
    ...
    it("should handle exercises with null/undefined skill level", async () => {
      const invalidExercises: Exercise[] = [
        createTestExerciseWithOverrides({
          ...testExercises[0],
          id: "null2",
          complexityLevel: null as any,
        }),
        createTestExerciseWithOverrides({
          ...testExercises[1],
          id: "undef2",
          complexityLevel: undefined as any,
        }),
        createTestExerciseWithOverrides({
          ...testExercises[2],
          id: "valid2",
      ...
      expect(result.filteredExercises.length).toBeGreaterThan(0);
      expect(result.filteredExercises.every((ex) => ex.complexityLevel)).toBe(
        true,
      );
    });
    ...
    it("should handle exercises with empty or null loadedJoints when joint restrictions exist", async () => {
      const jointTestExercises: Exercise[] = [
        createTestExerciseWithOverrides({
          ...testExercises[0],
          id: "j1",
          loadedJoints: null as any,
        }),
        createTestExerciseWithOverrides({
          ...testExercises[1],
          id: "j2",
          loadedJoints: undefined as any,
        }),
        createTestExerciseWithOverrides({
          ...testExercises[2],
          id: "j3",
      ...
      const hasEmptyJoints = result.filteredExercises.some(
        (ex) => !ex.loadedJoints || ex.loadedJoints.length === 0,
      );
  ...
  describe("Complex Filter Combinations", () => {
    ...
    it("should respect filter priority order with conflicting rules", async () => {
      ...
      const hasJointExercise = result.filteredExercises.some(
        (ex) => ex.name === "Included But Bad Joints",
      );
      expect(hasJointExercise).toBe(false);

      // Avoid overrides include
      const hasAvoidedExercise = result.filteredExercises.some(
        (ex) => ex.name === "Both Included and Avoided",
      );
    ...
    it("should handle case sensitivity in exercise names", async () => {
      ...
      const matchingExercises = result.filteredExercises.filter(
        (ex) => ex.name === "Barbell Squat",
      );
