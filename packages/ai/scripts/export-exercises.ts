...
async function exportAllExercises() {
  try {
    ...
    allExercises.forEach((ex) => {
      // Strength levels
      summary.byStrengthLevel[ex.strengthLevel] =
        (summary.byStrengthLevel[ex.strengthLevel] ?? 0) + 1;

      // Skill levels
      summary.bySkillLevel[ex.complexityLevel] =
        (summary.bySkillLevel[ex.complexityLevel] ?? 0) + 1;
      ...
      // Primary muscle
      summary.byPrimaryMuscle[ex.primaryMuscle] =
        (summary.byPrimaryMuscle[ex.primaryMuscle] ?? 0) + 1;

      // Fatigue profile
      summary.byFatigueProfile[ex.fatigueProfile] =
        (summary.byFatigueProfile[ex.fatigueProfile] ?? 0) + 1;
    });
