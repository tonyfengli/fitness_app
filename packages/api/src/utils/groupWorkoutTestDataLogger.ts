...
export class GroupWorkoutTestDataLogger {
  ...
  logClientProcessing(
    ...
    const essentialExercises = scoring.map((ex) =>
      this.extractEssentialExercise(ex),
    );
   ...
   */
  private extractEssentialExercise(
    ...
    return {
      ...
      scoreBreakdown: exercise.scoreBreakdown
        ? {
            ...
            muscleLessenPenalty:
              exercise.scoreBreakdown.muscleLessenPenalty || 0,
            intensityAdjustment:
              exercise.scoreBreakdown.intensityAdjustment || 0,
  ...
  logGroupExercisePools(
    ...
    for (const [blockId, exercises] of Object.entries(groupExercisePools)) {
      session.phases.groupPools[blockId] = {
        ...
        sharedByAll: exercises.filter(
          (ex) =>
            ex.clientsSharing.length === session.groupContext.clients.length,
        ).length,
        sharedBy2Plus: exercises.filter((ex) => ex.clientsSharing.length >= 2)
          .length,
        topShared: exercises
  ...
  logBlueprint(
    ...
    if (blueprint.blocks) {
      ...
      session.phases.blueprint = {
        type: "bmf",
        blocks: blueprint.blocks.map((block: any) =>
          this.extractBlockDebugData(block, session),
        ),
      ...
      session.phases.blueprint = {
        ...
        clientPools: Object.keys(blueprint.clientExercisePools).map(
          (clientId) => ({
            clientId,
            preAssigned:
              blueprint.clientExercisePools[clientId].preAssigned.length,
            availableCandidates:
              blueprint.clientExercisePools[clientId].availableCandidates
                .length,
            totalNeeded:
              blueprint.clientExercisePools[clientId].totalExercisesNeeded,
          }),
        ),
   ...
   */
  private extractBlockDebugData(
    block: GroupBlockBlueprint,
    session: any,
  ): BlockDebugData {
    return {
      ...
      individualCandidates: Object.entries(block.individualCandidates).map(
        ([clientId, data]) => {
          ...
          return {
            ...
            allExercises: data.exercises.map((ex) =>
              this.extractEssentialExercise(ex),
            ),
            totalFilteredCount:
              data.allFilteredExercises?.length || data.exercises.length,
          };
        },
      ),
    };
   ...
   */
  private extractEssentialGroupExercise(
    exercise: GroupScoredExercise,
    session: any,
  ): EssentialGroupExercise {
    return {
      ...
      clientScores: exercise.clientScores.map((cs) => ({
        ...
        clientName:
          session.groupContext.clients.find((c: any) => c.id === cs.clientId)
            ?.name || cs.clientId,
  ...
  async saveGroupWorkoutData(sessionId: string): Promise<void> {
    ...
    try {
      ...
      await this.saveFile(path.join(baseDir, `1-overview.json`), {
        sessionId,
        ...
        summary: {
          ...
          totalSharedExercises:
            session.phases.blueprint?.blocks.reduce(
              (sum: number, b: any) =>
                sum + b.sharedCandidates.topExercises.length,
              0,
            ) || 0,
        },
      });
      ...
      await this.saveFile(path.join(latestDir, "latest.json"), {
        overview: {
          sessionId,
          timestamp: session.timestamp,
          templateType: session.groupContext.templateType,
