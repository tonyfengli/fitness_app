...
Set your goals and preferences for today's session.`;
  ...
  startWorkoutLegacy: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      console.log(
        `[startWorkout] START - Session: ${input.sessionId} at ${new Date().toISOString()}`,
      );
      ...
      console.log(
        `[startWorkout] Template: ${session.templateType}, Already organized: ${!!session.workoutOrganization}`,
      );
      ...
      if (!isStandardTemplate) {
        ...
        console.log(
          `[startWorkout] SKIP Phase 2 - Non-standard template: ${session.templateType}`,
        );
        return {
          success: true,
      ...
      try {
        ...
        console.log(
          `[startWorkout] Found ${workoutsWithExercises.length} clients, ${workoutsWithExercises.reduce((sum, w) => sum + w.exercises.length, 0)} total exercises`,
        );
        ...
        console.log(
          `[startWorkout] Phase 2 input: ${phase2Input.clients.length} clients, ${phase2Input.exerciseCatalog.length} unique exercises`,
        );
...
- JSON only. Stop after JSON. End with: END`;
        ...
        console.log(
          `[startWorkout] Phase 2 Prompt - System: ${systemPrompt.length} chars, User: ${phase2Prompt.length} chars, Total: ${systemPrompt.length + phase2Prompt.length} chars`,
        );
        ...
        console.log(
          `[startWorkout] Phase 2 LLM call END - Duration: ${llmEndTime - llmStartTime}ms`,
        );

        // Log LLM metrics - focus on what matters for debugging
        if (response.usage_metadata) {
          console.log(
            `[startWorkout] Phase 2 LLM Usage - Input: ${response.usage_metadata.input_tokens}, Output: ${response.usage_metadata.output_tokens}, Reasoning: ${response.usage_metadata.reasoning_tokens || 0}, Total: ${response.usage_metadata.total_tokens}`,
          );
        }
        ...
        try {
          ...
          console.log(
            `[startWorkout] Phase 2 Response - Schema: ${parsedResponse.schemaVersion}, Assignments: ${parsedResponse.assignments?.length || 0}, Size: ${content.length} chars`,
          );
          ...
          console.error(
            "[startWorkout] Failed to parse LLM response:",
            parseError,
          );
        ...
        if (
          ...
          const validExerciseIds = new Set(
            phase2Input.exerciseCatalog.map((e) => e.id),
          );
          let typoCount = 0;
          ...
          for (const assignment of parsedResponse.assignments) {
            if (!validExerciseIds.has(assignment.exerciseId)) {
              ...
              if (bestMatch) {
                ...
                console.warn(
                  `[startWorkout] Fixed exercise ID typo for ${assignment.clientId}: "${assignment.exerciseId}" → "${bestMatch}" (${exerciseInfo?.name})`,
                );
                assignment.exerciseId = bestMatch;
                typoCount++;
              } else {
                console.error(
                  `[startWorkout] Could not fix invalid exercise ID for ${assignment.clientId}: "${assignment.exerciseId}"`,
                );
              }
            }
          }
          ...
          for (const assignment of parsedResponse.assignments) {
            ...
            assignedByClient
              .get(assignment.clientId)!
              .push(assignment.exerciseId);
          }
          ...
          for (const client of phase2Input.clients) {
            ...
            const assignedExercises =
              assignedByClient.get(client.clientId) || [];
            ...
            for (const exerciseId of expectedExercises) {
              expectedCounts.set(
                exerciseId,
                (expectedCounts.get(exerciseId) || 0) + 1,
              );
            }
            ...
            for (const exerciseId of assignedExercises) {
              assignedCounts.set(
                exerciseId,
                (assignedCounts.get(exerciseId) || 0) + 1,
              );
            }
            ...
            for (const [exerciseId, expectedCount] of expectedCounts) {
              ...
              if (missingInstances > 0) {
                ...
                console.error(
                  `[startWorkout] CRITICAL: Client ${client.clientName} missing ${missingInstances} instance(s) of exercise ${exerciseId} (${exerciseInfo?.name}) - This should not happen after typo fixes!`,
                );
          ...
          if (missingCount > 0) {
            console.error(
              `[startWorkout] WARNING: ${missingCount} exercises still missing after typo fixes - LLM may have completely omitted some exercises`,
            );
          }
        }
        ...
        if (
          parsedResponse.schemaVersion === "2.1" ||
          parsedResponse.schemaVersion === "2.2" ||
          parsedResponse.schemaVersion === "2.3" ||
          parsedResponse.schemaVersion === "2.5" ||
          parsedResponse.schemaVersion === "2.6" ||
          parsedResponse.schemaVersion === "2.6.2"
        ) {
          console.log(
            `[startWorkout] Transforming v${parsedResponse.schemaVersion} format to legacy format...`,
          );
          ...
          parsedResponse.assignments.forEach((assignment: any) => {
            ...
            const exerciseInfo = phase2Input.exerciseCatalog.find(
              (e: any) => e.id === assignment.exerciseId,
            );
          ...
          console.log(
            `[startWorkout] Transformed into ${rounds.length} rounds`,
          );
        } else {
        ...
        const totalExercisesProvided = phase2Input.clients.reduce(
          (sum, c) => sum + c.exercises.length,
          0,
        );
        const totalExercisesAssigned =
          parsedResponse.schemaVersion === "2.1" ||
          parsedResponse.schemaVersion === "2.2" ||
          parsedResponse.schemaVersion === "2.3" ||
          parsedResponse.schemaVersion === "2.5" ||
          ...
          parsedResponse.schemaVersion === "2.6.2"
            ...
            : roundOrganization.rounds.reduce((sum: number, round: any) => {
                return (
                  sum +
                  Object.values(round.exercisesByClient).reduce(
                    (rSum: number, exercises: any) => rSum + exercises.length,
                    0,
                  )
                );
              }, 0);
        ...
        if (totalExercisesProvided !== totalExercisesAssigned) {
          console.warn(
            `[startWorkout] WARNING: Exercise count mismatch - Provided: ${totalExercisesProvided}, Assigned: ${totalExercisesAssigned}`,
          );
        }
        ...
        for (const round of roundOrganization.rounds) {
          ...
          for (const [clientId, exercises] of Object.entries(
            ...
            if (!clientWorkout) {
              console.warn(
                `[startWorkout] No workout found for client ${clientId}`,
              );
              continue;
            }
            ...
            for (
              ...
              if (workoutExercise) {
                ...
                console.warn(
                  `[startWorkout] Exercise ${roundExercise.exerciseId} not found in workout`,
                );
              }
            }
          }
        ...
        console.log(
          `[startWorkout] Database updates END - Updated ${totalUpdates} exercises in ${Date.now() - dbUpdateStartTime}ms`,
        );

        console.log(
          `[startWorkout] END - Total duration: ${Date.now() - mutationStartTime}ms`,
        );
