...
import {
  getSessionUser,
  getSessionUserWithBusiness,
  getTrainerUser,
} from "../utils/session";
import {
  requireBusinessContext,
  requireTrainerRole,
  verifyClientInBusiness,
...
export const workoutRouter = {
  ...
  addExercises: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (
        workoutData.workout.userId !== currentUser.id &&
      ...
      if (
        currentUser.role === "trainer" &&
        workoutData.trainingSession.businessId !== currentUser.businessId
      ) {
        throw new TRPCError({
          code: "FORBIDDEN",
      ...
      const results = await ctx.db.transaction(async (tx) => {
        ...
        if (validExercises.length !== exerciseIds.length) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "One or more exercises are invalid or not available for your business",
          });
        }
      ...
      const result = await llmWorkoutService.saveIndividualWorkout(
        {
          ...
          workoutDescription:
            ...
            `Individual workout for ${client.name || client.email}`,
  ...
  updateExerciseOrder: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const exerciseIndex = workoutExercises.findIndex(
        (ex) => ex.id === input.workoutExerciseId,
      );
      if (exerciseIndex === -1) {
        throw new TRPCError({
          code: "NOT_FOUND",
  ...
  deleteWorkout: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await ctx.db.delete(Workout).where(eq(Workout.id, input.workoutId));
  ...
  addExercise: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const groupExercises = existingExercises.filter(
        (ex) => ex.groupName === input.groupName,
      );
      ...
      if (input.position === "beginning" && groupExercises.length > 0) {
        ...
        if (groupExercises.length > 0) {
          newOrderIndex =
            groupExercises[groupExercises.length - 1]!.orderIndex + 1;
        } else if (existingExercises.length > 0) {
          newOrderIndex =
            existingExercises[existingExercises.length - 1]!.orderIndex + 1;
        } else {
          newOrderIndex = 1;
        }

        // Insert the new exercise
        await ctx.db.insert(WorkoutExercise).values({
          workoutId: input.workoutId,
          exerciseId: input.exerciseId,
          orderIndex: newOrderIndex,
          setsCompleted: input.sets,
      ...
      const result = await ctx.db.transaction(async (tx) => {
        const [newWorkout] = await tx
          ...
          .values({
            ...
            notes:
              input.notes ||
              `Duplicated from workout on ${new Date().toLocaleDateString()}`,
