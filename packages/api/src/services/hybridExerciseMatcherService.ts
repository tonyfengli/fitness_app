...
export class HybridExerciseMatcherService {
  ...
  async matchExercises(
    ...
    const llmResult = await this.llmService.matchUserIntent(userPhrase, availableExercises, intent);
  ...
  private matchByPatterns(
    userPhrase: string,
    exercises: Pick<Exercise, 'id' | 'name' | 'exerciseType' | 'equipment' | 'movementPattern'>[]
  ): Omit<MatchResult, 'parseTimeMs'> {
    const normalized = userPhrase.toLowerCase().trim();
    ...
    if (normalized === 'bodyweight' || normalized === 'bodyweight exercises') {
      const matched = exercises.filter(ex => !ex.equipment || ex.equipment.length === 0);
      return {
        matchedExerciseNames: matched.map(ex => ex.name),
        matchedExercises: matched.map(ex => ({ id: ex.id, name: ex.name })),
        matchMethod: "pattern",
    ...
    if (normalized === 'dumbbells only' || normalized === 'only dumbbells' || normalized === 'dumbbell only') {
      const matched = exercises.filter(ex => 
        ex.equipment?.length === 1 && ex.equipment[0] === 'dumbbells'
      );
      return {
        matchedExerciseNames: matched.map(ex => ex.name),
        matchedExercises: matched.map(ex => ({ id: ex.id, name: ex.name })),
