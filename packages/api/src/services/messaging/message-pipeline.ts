...
import { UnifiedMessage, MessageResponse, MessageIntent, ProcessedMessage } from '../../types/messaging';
  ...
  async process(message: UnifiedMessage): Promise<ProcessedMessage> {
    ...
    try {
      ...
      if (!handler) {
        ...
        console.log(`[${new Date().toISOString()}] No handler for intent: ${intent.type}, using default response`);
        const response: MessageResponse = {
          success: true,
          message: "I'm not sure how to help with that. Please contact your trainer for assistance.",
          metadata: {
            intent: intent.type,
            unhandled: true
          }
      ...
      console.log(`[${new Date().toISOString()}] Routing to ${intent.type} handler`);
      ...
      try {
        ...
        unifiedLogger.logError('Failed to send error response', dispatchError, message);
      }
