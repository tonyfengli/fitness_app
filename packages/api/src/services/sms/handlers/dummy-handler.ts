...
export class DummyHandler {
  static handle(messageContent: string): SMSResponse {
    ...
    if (lowerContent.includes('check') || lowerContent.includes('here')) {
      return {
        success: true,
        message: "Thanks for checking in! Your session doesn't have a workout template configured. Please contact your trainer.",
        metadata: {
          handlerType: 'dummy',
          reason: 'no_template'
        }
    ...
    if (lowerContent.includes('help') || lowerContent.includes('?')) {
      return {
        success: true,
        message: "This session doesn't have preferences enabled. Just show up and your trainer will guide you!",
        metadata: {
          handlerType: 'dummy',
          reason: 'no_template'
        }
    ...
    return {
      success: true,
      message: "Message received. This session uses manual workout planning - see your trainer for details.",
      metadata: {
        handlerType: 'dummy',
        reason: 'no_template'
      }
  ...
  static shouldUseDummyHandler(templateType: string | null): boolean {
    ...
    if (!knownTemplates.includes(templateType)) {
      logger.info("Unknown template type - using dummy handler", { templateType });
      return true;
    }
